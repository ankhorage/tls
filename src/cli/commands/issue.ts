...
export const issue: AnkhCommandHandler = async (request) => {
  try {
    ...
    if (checks.some(({ status }) => status === 'fail')) {
      request.context.writeStderr('TLS issue aborted: fix the failed prerequisite(s) above and retry.\n');
      return { exitCode: 1 };
    }

    await issueCertificatesAsync(runtime, parsed);
    request.context.writeStdout(`Issued ${parsed.domains.length} certificate(s): ${parsed.domains.join(', ')}\n`);
    return { exitCode: 0 };
  } catch (error) {
...
function parseIssueArguments(argv: readonly string[]): ParsedIssueArguments {
  ...
  if (parsed.email === undefined) {
    throw new Error('Usage: ankh tls issue <domain...> --email <email> [--volume <name>] [--staging] [--force-renewal]');
  }
...
function parseTokens(
  ...
  if (token === '--staging') return parseTokens(argv.slice(1), { ...parsed, staging: true });
  if (token === '--force-renewal') return parseTokens(argv.slice(1), { ...parsed, forceRenewal: true });
  if (token === '--email' || token === '--volume') {
    if (value === undefined || value.startsWith('--')) throw new Error(`${token} requires a value.`);
    return parseTokens(rest, token === '--email' ? { ...parsed, email: value } : { ...parsed, volumeName: value });
  }
  if (token.startsWith('--')) throw new Error(`Unknown option: ${token}`);
  return parseTokens(argv.slice(1), { ...parsed, domains: [...parsed.domains, token] });
}
...
function renderChecks(checks: readonly TlsPreflightCheck[]): string {
  ...
  for (const item of checks) {
    ...
    if (item.status === 'fail' && item.tip !== undefined) lines.push(`  Tip: ${item.tip}`);
  }
  lines.push('');
  return lines.join('\n');
}
